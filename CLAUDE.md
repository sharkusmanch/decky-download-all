...
- **`RegisterForDownloadItems` delivers per-client *deltas*, not full snapshots.** Only the registration callback carries every client; later events carry just the clients that changed, so an event caused by another machine on the account has no wrapper for the local client at all. `parseDownloadItems` returns `items: null` for that case and `controller.ts` keeps its existing list — returning `[]` instead wipes the tracked downloads, and since Steam emits nothing while downloads sit idle, the plugin then stays blind (and silently logs `nothing to queue`) until the local client happens to appear in an event again. Verify with the CEF debugger by logging `args[1].map(e => e.remote_client_id)` per event.
